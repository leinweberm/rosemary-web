...
async fn update_image(image_uid: Uuid, data: PaintingImageUpdate) -> Result<impl Reply, Rejection> {
    ...
    let update_result = sqlx::query_as::<_, PaintingImage>(&query)
        .fetch_one(&client)
        .await;

    match update_result {
        Ok(image) => Ok(GenericResponse::send(
            Status::Success,
...
pub fn update() -> impl Filter<Extract = impl Reply, Error = Rejection> + Clone {
    warp::patch()
        ...
        .and(jwt_auth())
        .and_then(
            |image_uid: Uuid, data: PaintingImageUpdate, _claims: Claims| async move {
