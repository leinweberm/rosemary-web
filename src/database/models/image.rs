...
impl PaintingImage {
    ...
    pub fn create_query(data: PaintingImageCreate) -> String {
        format!(
            ...
            data.preview, data.alt_cs, data.alt_en, data.title_cs, data.title_en, data.painting_id
        )
    }
    ...
    pub fn update_query(data: PaintingImageUpdate, id: Uuid) -> String {
        ...
        if let (Some(cs), Some(en)) = (data.alt_cs.as_ref(), data.alt_en.as_ref()) {
            ...
            values.push(format!(
                "alt = JSONB_SET(alt::jsonb, '{{cs}}', '\"{}\"')",
                cs
            ));
        } else if let Some(en) = data.alt_en {
            values.push(format!(
                "alt = JSONB_SET(alt::jsonb, '{{en}}', '\"{}\"')",
                en
            ));
        };
        ...
        if let (Some(cs), Some(en)) = (data.title_cs.as_ref(), data.title_en.as_ref()) {
            ...
            values.push(format!(
                "title = JSONB_SET(title::jsonb, '{{cs}}', '\"{}\"')",
                cs
            ));
        } else if let Some(en) = data.title_en {
            values.push(format!(
                "title = JSONB_SET(title::jsonb, '{{en}}', '\"{}\"')",
                en
            ));
        }
    ...
    pub fn update_resized_query(id: Uuid, location: String, urls: Vec<String>) -> String {
        format!(
			...
			WHERE id = '{}'
			RETURNING *"#,
            location, urls[0], urls[1], urls[2], urls[3], id
        )
    }
...
fn try_from_json<T>(field: Option<Value>) -> Result<Option<T>, Error>
where
    T: DeserializeOwned,
{
