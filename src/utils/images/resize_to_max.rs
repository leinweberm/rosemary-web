...
async fn resize_to_max_image(
	...
	let file = match image_file {
		...
		Err(join_error) => {
			...
			return Err(ImageError::IoError(io::Error::new(io::ErrorKind::Other, format!("{}", join_error))))
		}
	};
	...
	let resized_file = task::spawn_blocking(move || file.resize(
		new_width,
		new_height,
	...
	let resized = match resized_file {
		...
		Err(join_error) => {
			...
			return Err(ImageError::IoError(io::Error::new(io::ErrorKind::Other, "failed to resize image")))
		}
	};

	let resize_result = task::spawn_blocking(move || {
		resized.to_rgb8().save_with_format(&new_path_string, ImageFormat::Jpeg)?;
	...
	match resize_result {
		...
		Ok(Err(image_error)) => {
			...
			Err(ImageError::IoError(io::Error::new(io::ErrorKind::Other, format!("{}", image_error))))
		},
		Err(join_error) => {
			error!(target: "app", "images:resize - failed to save resized image {}", join_error);
			Err(ImageError::IoError(io::Error::new(io::ErrorKind::Other, format!("{}", join_error))))
		}
	}
}
	...
	for width in job.max_sizes.into_iter() {
		...
		if let Some(name) = &job.new_name {
			let temp_new_file_path = Path::new(&static_dir_path)
				.join(format!("images/{}_{}.jpeg", &name, width));
