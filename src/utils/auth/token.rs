...
pub async fn encode_token(
    ...
    headers.insert(
        ...
        HeaderValue::from_str(&expires_in.to_string()).unwrap(),
    );
    headers.insert(
        "X-Token-Valid-Until",
...
pub async fn decode_token(token: String, secret: &JwtKeys) -> Result<Claims, Rejection> {
    match decode::<Claims>(&token, &secret.decoding, &Validation::new(Algorithm::HS256)) {
        Ok(token_data) => {
            let current_time = SystemTime::now()
                .duration_since(UNIX_EPOCH)
...
mod tests {
    ...
    async fn test_jwt_token() {
        ...
        let token = encode_token(&user_id.to_string(), expiration, &keys)
            .await
            .unwrap();
